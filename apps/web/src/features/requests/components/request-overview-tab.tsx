...
export function RequestOverviewTab({
  ...
  const submissions = useSubmissions(request.id, true);
    ...
    <div className="space-y-4">
      ...
      <section className="rounded-lg border border-border bg-card shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-4 py-3">
          ...
          {(canUpdate || canAssign || canSubmitReview) && (
          ...
          <DetailCell
            ...
            value={
              request.dueDate ? (
                <span
                  className={
                    request.isOverdue ? "font-medium text-destructive" : undefined
                  }
                >
          ...
          {request.assignees.length === 0 ? (
            ...
            <ul className="flex flex-wrap gap-2">
              {request.assignees.map((a) => (
                <li
                  key={a.userId}
                  className="inline-flex items-center gap-2 rounded-md border border-border bg-muted/30 px-2 py-1.5 text-sm"
                >
                  <UserAvatar
                    src={a.avatarUrl}
      ...
      {canViewDocuments ? <RequestWorkingPapers request={request} /> : null}
...
function RequestWorkingPapers({ request }: { request: RequestDetail }) {
  ...
  const documents = useDocumentsList(query);
  const documentsHref =
    `/engagements/${request.engagementId}/documents?category=WorkingPaper` +
  ...
  return (
    <section className="rounded-lg border border-border bg-card shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-4 py-3">
        <div>
          <h3 className="text-sm font-semibold">Working papers</h3>
          <p className="text-xs text-muted-foreground">
