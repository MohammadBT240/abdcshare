...
- Reading routes carry no `@RequirePermission` — every authenticated role can reach them; visibility is filtered by `visibleToRoles` (empty array = all roles) and `status === 'published'`. The single-article lookup (`getArticleBySlug` / `GET /help/articles/:slug`) is the ONE exception: a caller with `help:manage` may preview a draft there, for the "does my just-authored article render correctly" workflow (e.g. via a `HelpTip` pointed at its slug before publishing). The category-tree and search endpoints (`getCategoriesForViewer`, `searchArticles`) stay published-only for every caller, managers included — per the design spec (`docs/superpowers/specs/2026-09-01-help-center-design.md` lines 96-99), browsing/finding drafts is what the separate, unfiltered `/help/admin/*` routes (Task 6) are for.
