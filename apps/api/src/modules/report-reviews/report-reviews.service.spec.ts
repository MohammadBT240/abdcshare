...
function build(doc: Record<string, unknown>, cycle: Record<string, unknown>) {
  const em = {
    findOne: jest.fn(async (entity: unknown) =>
      entity === DocumentEntity ? doc : entity === ReportReviewCycleEntity ? cycle : null,
    ),
    find: jest.fn(async () => [cycle]),
...
const baseDoc = () => ({
  ...
  engagement: { id: 'e1', team: { getItems: () => [] } },
});
...
describe('ReportReviewsService.respond', () => {
  it('client approval finalises the report (SignedOff)', async () => {
    ...
    const cycle = { decision: ReportReviewDecision.Pending };
  ...
  it('changes requested on round < 3 → ChangesRequested', async () => {
    ...
    expect(doc.status).toBe(DocumentStatus.Ready); // not finalised
  });
