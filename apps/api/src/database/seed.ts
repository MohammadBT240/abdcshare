...
import { FsLineEntity } from '../modules/fs-lines/infrastructure/persistence/fs-line.entity';
...
const FS_LINES = ['Cash & Bank', 'Receivables', 'Payables', 'Revenue', 'PP&E', 'Inventory'];
...
export async function runSeed(orm: MikroORM): Promise<void> {
  ...
  for (const name of FS_LINES) await ensure(em, FsLineEntity, { name }, { name });
